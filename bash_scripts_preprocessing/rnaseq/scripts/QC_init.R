...
cvge <- cvge[cvge$Sample %in% md$Sample,]
rownames(cvge) <- cvge$Sample
rownames(md) <- md$Sample
#cts <- data.frame(gene = cts$gene, cts[,colnames(cts) %in% md$Sample])

md <- md[which(colnames(cts) %in% md$Sample),]
...
#ordered_st <- st_sts[,md$Sample]
#st_sts <- cbind(fullst, ordered_st)
st_sts[,2:ncol(st_sts)] <- st_sts[,md$Sample]
#print(colnames(st_sts)[2:ncol(st_sts)] %in% md$Sample)
print(md$Sample)
stopifnot(colnames(st_sts)[2:ncol(st_sts)] %in% md$Sample)
...
iv <- match(md$Sample, cvge$Sample)
...
iv <- match(forPlot$Sample, md$Sample)
head(md[iv,])
forPlot$Status <- md[iv,]$Group
forPlot$Sample_ID <- md[iv,]$Sample
#forPlot$Set <- md[iv,]$Type
...
md$Sample <- factor(md$Sample, levels = md$Sample)
inputReadsBar <- ggplot(data=md, mapping=aes(x=Sample,y=input_reads,fill=Group)) +
  geom_bar(stat="identity") +
...
raw <- PCX_counts
raw <- raw[,md$Sample]
stopifnot(colnames(raw)==md$Sample)
raw <- raw[,order(colnames(raw))]
md$libSize <- colSums(raw)

uniqueReadsBar <- ggplot(data=md, mapping=aes(x=Sample,y=libSize,fill=Group)) +
  geom_bar(stat="identity") +
...
iv <- match(my.pca$Sample, md$Sample)
potential_batch <- c("input_reads","libSize","exon_fraction","PASS","Group")
my.pca <- data.frame(my.pca, md[iv,potential_batch])

ggplot(data = my.pca, mapping = aes(x = PC1,y = PC2, colour = Group, label=TRUE)) +
  geom_point(size = 2) +
  theme_bw() +
...
for (i in 1:length(potential_batch)) {
  if (i==1) {
    p <- ggplot(data = my.pca, mapping = aes(x = PC1,y = PC2, colour = input_reads)) +
      geom_point(size = 3) +
      theme_bw() +
      xlab(paste0("PC-1 (", propPC1, "%)")) +
      ylab(paste0("PC-2 (", propPC2, "%)"))
...
sub_cts <- cts[,md$Sample[which(md$PASS == TRUE)]]
