...
rule star:
    ...
    output:
        ...
        "samples/star/{sample}_bam/Chimeric.out.junction"
    params:
        gtf=config["gtf_file"]
    ...
    run:
         ...
         shell("""
                {STAR} --runThreadN 5 --runMode alignReads --genomeDir {pathToGenomeIndex} \
...
rule picard:
    ...
    run:
      shell("java -Xmx7g -jar {params.picard} MarkDuplicates \
      INPUT={input} \
      OUTPUT={output} \
